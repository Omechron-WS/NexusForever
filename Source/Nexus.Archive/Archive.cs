...
public sealed class Archive : IDisposable
{
    ...
    public Stream OpenFileStream(IArchiveFileEntry fileEntry, bool handleCompression = true)
    {
        ...
        Stream baseStream = ArchiveFile?.Open(fileEntry)
            ?? CoreDataArchive?.Open(fileEntry)
            ?? OpenLocalFile(fileEntry.Path);
    ...
    internal static Stream HandleCompression(
        ...
        Stream baseStream)
    {
        ...
        switch (flags & (ArchiveFileFlags.CompressedLzma | ArchiveFileFlags.CompressedDeflate))
        {
            case ArchiveFileFlags.CompressedLzma:
                if (compressedSize < 5L || uncompressedSize < 0L)
                    throw new InvalidDataException("The LZMA archive entry has invalid compressed or uncompressed sizes.");
                ...
                return LzmaStream.Create(
                    properties,
                    baseStream,
                    compressedSize - properties.Length,
                    uncompressedSize);
            case ArchiveFileFlags.CompressedDeflate:
                return new DeflateStream(baseStream, CompressionMode.Decompress, false);
            case ArchiveFileFlags.CompressedLzma | ArchiveFileFlags.CompressedDeflate:
                throw new InvalidDataException("The archive entry declares multiple compression formats.");
            default:
                return baseStream;
        }
    }
