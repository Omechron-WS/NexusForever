...
{
    public class PrerequisiteCheckTrueLevelTests
    {
        [Fact]
        public void AddGamePrerequisiteDiscoversTrueLevelCheck()
        {
            ...
            IPrerequisiteCheck check = serviceProvider.GetRequiredKeyedService<IPrerequisiteCheck>(
                PrerequisiteType.TrueLevel);

            Assert.IsType<PrerequisiteCheckTrueLevel>(check);
        }
        ...
        public void MeetsAndTryMeetsCompareUnsignedLevelAndIgnoreObject(
            ...
            bool expected)
        {
            ...
            player.SetupGet(entity => entity.Level).Returns(currentLevel);
            var unit = new Mock<IUnitEntity>(MockBehavior.Strict);
            unit.SetupGet(entity => entity.Level).Returns(currentLevel);
            ...
            player.VerifyGet(entity => entity.Level, Times.Once);
            unit.VerifyGet(entity => entity.Level, Times.Once);
        ...
        private static PrerequisiteCheckTrueLevel CreateCheck()
        {
            return new PrerequisiteCheckTrueLevel(
                Mock.Of<ILogger<PrerequisiteCheckTrueLevel>>());
        }
    }
}
