...
| 1 — Loot | Incomplete | Schema, recursive loading, startup/ticking, owner attachment, authorised same-map collection, reconnect and owner-scoped removal notification, loot bags, capacity-safe item grants, expiry, stale-owner cleanup, final-package corpse cleanup, and fail-closed class/race/level/quest conditions are live and tested. A validated world-local group snapshot cache now preserves the last delivered group state, but it lacks source revisions and is not sufficient as sole loot authority. Group/raid allocation remains incomplete, and retail loot seed data is unavailable. |
| 2 — Spell variants | Incomplete | Factory dispatch, failed-cast cleanup, cancellation, phase masks, central delayed/periodic effect timing, and aura target cleanup are live and tested. Threshold-child casting and several specialised variant semantics remain incomplete. |
...
| 6 — Entity vitals and lifecycle | In progress | Wire-compatible vital aliases, focus property data, persistence-safe initial pools, deterministic unit/class regeneration, idempotent death and rewards, corpse cleanup, persistent non-player respawn, transactional build-16042 base and activation-batch per-effect vital costs, conservative fixed-value VitalModifier effects, and a bounded central effect timeline are live and tested. VitalModifier formula/range rows, threshold-child casting, sprint, dash, and the remaining class-resource action hooks are incomplete. |
| 7 — Quests | In progress | Quest and achievement persistence is commit-aware. Item, money, XP, and reputation rewards are prevalidated with aggregate inventory exchange and duplicate-completion guards. Persisted objectives are validated and ordered; ordinary, dynamic, checklist, optional, sequential, empty-quest, nested target-group, communicator delivery/conditions, callback pickup/hand-in, secure group sharing, quest timers, repeat resets, and the QuestAdvanceObjective, CollectItem, and final CompleteQuest adapters are live and tested. Remaining event adapters, objective-timer lifecycle, the ambiguous client communicator action, remaining reward domains, and durable cross-domain reward delivery remain incomplete. |
...
- World servers retain deeply immutable, validated group snapshots from internal group events, reject malformed identities, flags, loot rules, and removal reasons, and linearise disband against local publication. Disband tombstones are bounded; without source revisions or a broker ordering guarantee the cache deliberately represents only the last validated delivered state and cannot be the sole group-loot authority.
...
- Finish the remaining Phase 2 spell variants, unit-capable effect prerequisites, VitalModifier formula/range semantics, and persistent-effect lifecycle before relying on threshold or formula-driven effects for broader scripted combat content.
- Add monotonic group revisions or an equivalent ordered incarnation before using cached membership as the sole authority for group/raid loot allocation.
...
1. Close the remaining Phase 4 unit-capable effect-prerequisite gap and Phase 6 VitalModifier formula/range work, then complete threshold casting, dash, and sprint.
...
3. Expand the reactive NPC pilot only where build-16042 profile, proximity, line-of-sight, and movement evidence is available; keep unsupported Ravenok proc/multiphase attacks gated behind effect prerequisites and threshold semantics.
4. Version group state before group/raid loot allocation, make cross-server lifecycle publication retryable, and continue lower-risk long-uptime hardening alongside the gameplay phases.
